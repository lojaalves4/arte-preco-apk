...
def calcular_preco(produto: str, material: float, horas: float, valor_hora: float,
                   despesas: float, margem: float, validade_dias: int) -> dict:
    ...
    data_validade = data_emissao + timedelta(days=max(0, int(validade_dias)))
